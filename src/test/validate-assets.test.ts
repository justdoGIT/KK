...
describe("repository-controlled SVG assets", () => {
  ...
  it.each([
    ...
    ["quoted greater-than external href", '<svg><image foo=">" href="https://evil.example/x" /></svg>', "external or data references are not allowed"],
    ...
    ["style-attribute external url", '<svg><rect style="fill:url(https://evil.example/pattern.svg)" /></svg>', "external or data CSS references are not allowed"],
